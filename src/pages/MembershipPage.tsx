...
export default function MembershipPage() {
  ...
  return (
    <>
      <PageHero eyebrow={tr(locale, "Membership")} title={tr(locale, "Be Part of the Journey")} copy="A clear membership page for people who want to learn astronomy, attend observations, support outreach and grow with the Samanta Chandra Shekhar Amateur Astronomers' Association (SCAAA) community." image={imagery.telescope} />
      <section className="section">
        <SectionHeader title={tr(locale, "Why Join SCAAA?")} />
        ...
        <div className="card-grid two">
          {membershipTypes.map((type) => (
            <article className="program-card" key={type.id}>
              ...
              <button className="button">{tr(locale, "Join SCAAA")}</button>
            </article>
          ))}
        </div>
      </section>
